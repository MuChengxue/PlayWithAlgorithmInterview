...
public class LCS1 {
    ...
    public String lcs(String s1, String s2) {
        ...
        lcs(s1, s2, s1.length() - 1, s2.length() - 1, memo);
        return getLCS(s1, s2, memo);
    }
    ...
    private int lcs(String s1, String s2, int m, int n, int[][] memo) {
        ...
        if (s1.charAt(m) == s2.charAt(n))
            res = 1 + lcs(s1, s2, m - 1, n - 1, memo);
        else
            res = Math.max(lcs(s1, s2, m - 1, n, memo),
    ...
    private String getLCS(String s1, String s2, int[][] memo) {
